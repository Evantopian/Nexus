...
const PlayerRecommendation = ({
  ...
  useEffect(() => {
    if (selectedGames.length === 0) {
      ...
      const filtered = recommendedPlayers?.filter((_, index) => {
        ...
        const hash =
          selectedGames.reduce((acc, gameId) => acc + gameId.charCodeAt(0), 0) +
          index;
  ...
  const prev = () =>
    setActiveIndex(
      (i) => (i - 1 + filteredPlayers.length) % filteredPlayers.length
    );
  ...
  if (!filteredPlayers || filteredPlayers.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center">
        <div className="bg-gray-200 dark:bg-gray-700/50 rounded-full p-4 mb-4">
          <ChevronRight
            size={24}
    ...
    <div className="relative h-full flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md mx-auto h-full">
        {filteredPlayers.map((player, idx) => {
          ...
          const isPrev =
            idx ===
            (activeIndex - 1 + filteredPlayers.length) % filteredPlayers.length;
          ...
          if (isActive) {
            ...
            position =
              "-translate-y-[40%] -translate-x-[15%] scale-80 opacity-60";
            zIndex = 20;
          } else if (isNext) {
            position =
              "translate-y-[40%] translate-x-[15%] scale-80 opacity-60";
          ...
          return (
            <div
              ...
              style={{ zIndex }}
            >
              <div className="bg-white dark:bg-gray-800 rounded-xl overflow-hidden border border-gray-300 dark:border-gray-600 shadow-xl w-full max-w-xs">
                {/* Player Card Header */}
                ...
                <div className="pt-12 pb-4 px-4 text-center">
                    ...
                    <div className="mb-4">
                      ...
                      <div className="flex flex-wrap justify-center gap-1">
                        {selectedGames.map((gameId, index) => (
                          <div
                            key={gameId}
                            className="flex items-center bg-gray-200 dark:bg-gray-600 rounded-full px-2 py-0.5"
                          >
                            <GameController
                              size={10}
                              className="text-teal-500 dark:text-teal-400 mr-1"
                            />
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    <div className="bg-gray-100 dark:bg-gray-700 rounded-lg p-2">
                      <div className="text-teal-500 dark:text-teal-400 text-base font-bold">
      ...
      {filteredPlayers.length > 1 && (
        <>
          <button
            onClick={prev}
            className="absolute left-0 top-1/2 -translate-y-1/2 p-2 rounded-full bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 z-40"
          >
            <ChevronLeft className="w-4 h-4 text-gray-800 dark:text-white" />
          </button>
          <button
            onClick={next}
            className="absolute right-0 top-1/2 -translate-y-1/2 p-2 rounded-full bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 z-40"
          >
