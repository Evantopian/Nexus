...
import { GET_PARTY_BY_USER, GET_PARTY_INVITATIONS, GET_SENT_PARTY_INVITATIONS } from "@/graphql/party/partyQueries"
import {
  CREATE_PARTY,
  DELETE_PARTY,
  HANDLE_PARTY_INVITE,
...
import { Users, GamepadIcon as GameController, UserPlus, Search } from "lucide-react"
...
const Party = () => {
  ...
  const [activeTab, setActiveTab] = useState<"recommendations" | "invites">("recommendations")
  ...
  const partyMemberIds = partyData?.getPartyByUser?.members?.map((member: PartyMember) => member.uuid) || []

  // Filter out party members from recommendations
  const filteredRecommendations = recommendedPlayers?.filter((player: Player) => !partyMemberIds.includes(player.id))

  const { data: receivedInvitesData, refetch: refetchReceivedInvites } = useQuery(GET_PARTY_INVITATIONS, {
    variables: { userId: user?.uuid },
    skip: !user?.uuid,
  })

  const { data: sentInvitesData, refetch: refetchSentInvites } = useQuery(GET_SENT_PARTY_INVITATIONS, {
    variables: { userId: user?.uuid },
    skip: !user?.uuid,
  })
      ...
      <div className="max-w-[1800px] mx-auto px-4 py-6">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="w-full lg:w-1/3 flex flex-col gap-4">
            ...
            <div className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700">
              <div className="p-3 bg-gray-700 flex items-center justify-between">
                ...
                {partyData?.getPartyByUser && (
                  <span className="text-xs bg-teal-500 px-2 py-0.5 rounded-full">
                    {partyData.getPartyByUser.members?.length || 0}/{partyData.getPartyByUser.maxMembers || 5}
                  </span>
                )}
              </div>
            ...
            {/* Games Section */}
            <div className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700 flex-grow">
              <div className="p-3 bg-gray-700 flex items-center justify-between">
                <div className="flex items-center">
                  <GameController className="w-5 h-5 mr-2 text-teal-400" />
                  <h2 className="text-base font-bold">Games</h2>
                </div>
                <span className="text-xs bg-teal-500 px-2 py-0.5 rounded-full">{filteredGames?.length || 0}</span>
              </div>
              ...
              <div className="p-3">
                <div className="relative mb-3">
                  ...
                  <input
                    ...
                    className="w-full bg-gray-700 border border-gray-600 rounded-lg py-2 pl-10 pr-3 text-sm focus:outline-none focus:ring-1 focus:ring-teal-500"
                  />
                </div>
                ...
                <div className="h-[calc(100vh-400px)] overflow-y-auto pr-1 space-y-2 custom-scrollbar">
                  {gamesLoading ? (
                    ...
                    filteredGames.map((game) => (
                      ...
                      >
                        <div className="flex items-center">
                          <div className="w-10 h-10 rounded-lg overflow-hidden mr-3 bg-gray-600 flex-shrink-0">
                            {game.image ? (
                              <img
          ...
          <div className="w-full lg:w-2/3">
            <div className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700 h-full">
              <div className="p-3 bg-gray-700 flex items-center justify-between">
                <div className="flex items-center">
                  <UserPlus className="w-5 h-5 mr-2 text-teal-400" />
                ...
                <div className="flex space-x-1">
                  ...
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded-lg transition ${
                      activeTab === "invites" ? "bg-teal-600 text-white" : "bg-gray-600 text-gray-300 hover:bg-gray-500"
                    }`}
                  >
                    Invites
                  </button>
              ...
              {selectedGames.length > 0 && (
                <div className="px-3 py-2 bg-gray-700/50 flex flex-wrap gap-2">
                  <span className="text-xs text-gray-400">Selected games:</span>
